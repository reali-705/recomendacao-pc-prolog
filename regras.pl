% SISTEMA DE RECOMENDACAO DE COMPONENTES PARA PC - REGRAS ESSENCIAIS
% Arquivo: regras.pl
% Contem todas as regras essenciais do projeto

:- use_module(library(lists)).

:- ensure_loaded(dados).

% --- REGRA GERAL PARA CONSULTA POR PRECO ---
...
% --- REGRAS PARA RECOMENDACAO COMPLETA DE PC (OPCAO 1) ---

% REGRA PRINCIPAL: Recomendacao completa de PC
pc_recomendacao(OrcamentoTotal, Resolucao, [PC]) :-
    % 1. Obter requisitos minimos para a resolucao
    requisitos_resolucao(Resolucao, G3DMark_Min, RAM_Min, SSD_Min, Fonte_W_Min, CPU_Min),

    % 2. Encontrar componentes que atendam aos requisitos minimos
    ...
    % 3. Verificar compatibilidade basica
...
% REGRA PARA BUSCAR AS MELHORES RECOMENDACOES
...
% Ordena pela diferenca do orcamento
...
% REGRA PARA MOSTRAR RECOMENDACOES DE FORMA LEGIVEL
mostrar_recomendacao(pc(Res, Total, Diff, 
    ...
    format('~n=== RECOMENDACAO PARA ~w ===', [Res]),
    format('~nPreco Total: R$ ~2f (Diferenca do orcamento: R$ ~2f)', [Total, Diff]),
